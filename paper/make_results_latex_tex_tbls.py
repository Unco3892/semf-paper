    ...
    try:
        if mean_std:
            ...
            if percentage:
                ...
                formatted_val = f"{mean_val}\\%±{std_val}\\%"
                if mean_val > 0:
...
def aggregate_metrics(data):
    ...
    std_data = data.groupby('dataset', observed=True).std().reset_index()
...
def generate_caption(model_class, missing_rate, aggregate, seeds_str, num_seeds):
    ...
    aggregate_text = "aggregated over {num_seeds} seeds, with rows ordered by seed (ascending). For each metric, the mean and standard deviation of the performance across the seeds are separated by ±." if aggregate else f"for seeds {seeds_str}, with rows ordered by seed (ascending)."
...
def generate_tables(data, aggregate=True, output_dir=None):
    """
    ...
    for model_class in model_classes:
        for missing_rate, label_suffix in zip([0, 0.5], ['complete', 'missing']):
            ...
            filtered_data = filtered_data.sort_values('dataset')
...
if __name__ == '__main__':
    ...
    file_path = os.path.join(
        # new_dir, "results", "sweep_best_params.csv"
        new_dir, "results", "sweep_results_complete.csv"
    ...
    generate_tables(data, aggregate=True, output_dir=output_dir)
    
    # Optionally, generate LaTeX tables for all model classes without aggregation
    generate_tables(data, aggregate=False, output_dir=output_dir)
