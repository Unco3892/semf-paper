    ...
    for l in range(n_outcomes):
        for r in range(sampling_R):
            try:
                ...
                # # Generate random values one sample at a time to avoid broadcasting issues
                # random_values = np.random.normal(loc=mean, scale=std)
                # desired_z_R[:, l, r] = random_values
